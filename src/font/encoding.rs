...
impl Encoding {
    ...
    pub fn win_ansi() -> Self {
        ...
        table[0x9F] = '\u{0178}'; // Ÿ
        Self {
            base: Some(BaseEncoding::WinAnsiEncoding),
            table,
            glyph_names: std::array::from_fn(|_| None),
        }
    }
    ...
    pub fn mac_roman() -> Self {
        ...
        const MAC_ROMAN_HIGH: [char; 128] = [
            ...
            '¢', '£', '§', '•', '¶', 'ß', '®', '©', '™', '´', '¨', '\u{2260}', 'Æ', 'Ø',
            '\u{221E}', '±', '\u{2264}', '\u{2265}', '¥', 'µ', '\u{2202}', '\u{2211}', '\u{220F}',
            'π', '\u{222B}', 'ª', 'º', '\u{03A9}', 'æ', 'ø', '¿', '¡', '¬', '\u{221A}', '\u{0192}',
            '\u{2248}', '\u{2206}', '«', '»', '\u{2026}', '\u{00A0}', 'À', 'Ã', 'Õ', 'Œ', 'œ',
            '\u{2013}', '\u{2014}', '\u{201C}', '\u{201D}', '\u{2018}', '\u{2019}', '\u{00F7}',
            '\u{25CA}', 'ÿ', '\u{0178}', '\u{2044}', '\u{20AC}', '\u{2039}', '\u{203A}',
            '\u{FB01}', '\u{FB02}', '\u{2021}', '·', '\u{201A}', '\u{201E}', '\u{2030}', 'Â', 'Ê',
            'Á', 'Ë', 'È', 'Í', 'Î', 'Ï', 'Ì', 'Ó', 'Ô', '\u{F8FF}', 'Ò', 'Ú', 'Û', 'Ù', 'ı',
            '\u{02C6}', '\u{02DC}', '\u{00AF}', '\u{02D8}', '\u{02D9}', '\u{02DA}', '\u{00B8}',
            '\u{02DD}', '\u{02DB}', '\u{02C7}',
        ];
        for (i, &c) in MAC_ROMAN_HIGH.iter().enumerate() {
            table[0x80 + i] = c;
        }
        Self {
            base: Some(BaseEncoding::MacRomanEncoding),
            table,
            glyph_names: std::array::from_fn(|_| None),
        }
    }
    ...
    pub fn standard() -> Self {
        ...
        Self {
            base: Some(BaseEncoding::StandardEncoding),
            table,
            glyph_names: std::array::from_fn(|_| None),
        }
    }
    ...
    pub fn font_specific() -> Self {
        ...
        Self {
            base: Some(BaseEncoding::FontSpecific),
            table,
            glyph_names: std::array::from_fn(|_| None),
        }
    }
    ...
    pub fn from_encoding_dict(dict: &CosDictionary) -> Self {
        ...
        let mut enc = match dict
            .get(&CosName::new(b"BaseEncoding".to_vec()))
...
mod tests {
    ...
    fn base_encoding_from_name() {
        assert_eq!(
            ...
            Some(BaseEncoding::WinAnsiEncoding)
        );
