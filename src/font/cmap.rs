                ...
                while i < tokens.len() && tokens[i] != "endbfchar" {
                    if i + 1 < tokens.len() {
                        if let (Some(src), Some(dst)) = (
                            ...
                            parse_hex_to_unicode(&tokens[i + 1]),
                        ) {
                        ...
                        if let (Some(s), Some(e)) = (start, end) {
                            if base_tok.starts_with('[') {
                                for (off, hex) in parse_array_token(&base_tok).iter().enumerate() {
                                    ...
                                    if let Some(u) = parse_hex_to_unicode(hex) {
                                        cmap.mappings.insert(code, u);
                                    }
                                }
    ...
    match bytes.len() {
        ...
        4 => {
            ...
            if (0xD800..=0xDBFF).contains(&u0) && (0xDC00..=0xDFFF).contains(&u1) {
                ...
                Some(format!(
                    "{}{}",
                    char::from_u32(u0 as u32)?,
                    char::from_u32(u1 as u32)?
                ))
            }
        }
        _ => {
            ...
            let mut i = 0;
            while i + 1 < bytes.len() {
                if let Some(c) = char::from_u32(u16::from_be_bytes([bytes[i], bytes[i + 1]]) as u32)
                {
                    s.push(c);
                }
                i += 2;
...
mod tests {
    ...
    fn parse_bfrange_sequential() {
        let c =
            parse_to_unicode_cmap(wrap("1 beginbfrange\n<30><39><0030>\nendbfrange\n").as_bytes());
    ...
    fn unknown_code_returns_none() {
        assert_eq!(
            parse_to_unicode_cmap(b"begincmap\nendcmap\n").to_unicode(0xFF),
            None
        );
    }
    #[test]
