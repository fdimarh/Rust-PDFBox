...
use bcder::{Captured, Integer, Mode, OctetString, Oid, Tag, encode::PrimitiveContent};
...
pub fn fetch_ocsp_response(cert: &CapturedX509Certificate, ocsp_url: &str) -> Option<Vec<u8>> {
    use x509_parser::prelude::*;
    ...
    if resp.status().is_success() {
        ...
        eprintln!(
            "[ltv] OCSP request to {ocsp_url} failed: HTTP {}",
            resp.status()
        );
        None
    }
...
pub fn fetch_crl_response(crl_url: &str) -> Option<Vec<u8>> {
    ...
    if resp.status().is_success() {
        ...
        eprintln!(
            "[ltv] CRL request to {crl_url} failed: HTTP {}",
            resp.status()
        );
        None
    }
...
pub fn encode_revocation_info_archival(
    ...
    if !crls.is_empty() {
        let crl_items: Vec<CrlBytes> = crls
            .into_iter()
            .map(|b| CrlBytes {
    ...
    if !ocsps.is_empty() {
        ...
        for ocsp_bytes in ocsps {
            ...
            let status_enum = Integer::from(0u8)
...
pub fn inject_unsigned_attribute_into_cms(
    ...
    let err = |msg: &str| PdfError::Parse {
        offset: None,
    ...
    let read_tl = |data: &[u8], pos: usize| -> Option<(u8, usize, usize)> {
        ...
        if lb < 0x80 {
            ...
            if nb == 0 || nb > 4 || pos + 2 + nb > data.len() {
                return None;
            }
    ...
    let mut new_si_content =
        Vec::with_capacity(si_before.len() + 1 + ua_len_bytes.len() + new_ua_content.len());
...
pub fn build_adbe_revocation_unsigned_der(
    ...
    let attr = bcder::encode::sequence((oid.encode(), bcder::encode::set(rev_raw)));
...
pub fn append_dss_dictionary(
    ...
    for crl in crl_data {
        ...
        dict.set(
            CosName::new(b"Length"),
            CosObject::Integer(crl.len() as i64),
        );
    ...
    for ocsp in ocsp_data {
        ...
        dict.set(
            CosName::new(b"Length"),
            CosObject::Integer(ocsp.len() as i64),
        );
    ...
    for cert in chain {
        ...
        dict.set(
            CosName::new(b"Length"),
            CosObject::Integer(der.len() as i64),
        );
    ...
    IncrementalWriter::write_update(
        &pdf_bytes,
        &doc,
        &changed,
...
pub fn fetch_timestamp_token(tsa_url: &str, message_digest: &[u8]) -> Result<Vec<u8>, PdfError> {
    let to_err = |msg: String| PdfError::Parse {
        offset: None,
        context: msg,
    };
