...
impl SignatureValidator {
    ...
    pub fn validate(
        pdf_bytes: &[u8],
        password: Option<&str>,
    ) -> Result<Vec<ValidationResult>, PdfError> {
        ...
        for r in &mut results {
            ...
            if !bv {
                r.errors
                    .push("ByteRange structure invalid (possible USF attack)".into());
            }
            ...
            if !nw {
                r.errors
                    .push("Signature Contents may be relocated (possible SWA attack)".into());
            }
            ...
            if !co {
                r.errors
                    .push("Document certification permissions violated".into());
            }
        }
            ...
            && !results[last].no_unauthorized_modifications
        {
            results[last]
                .errors
                .push("ByteRange does not cover the entire file".into());
        }
...
fn collect_sig_fields(doc: &Document) -> Vec<SigField> {
    ...
    while let Some(item) = queue.pop() {
        let fid = match item.as_reference() {
            Some(r) => r,
            None => continue,
        };
        ...
        if let Some(kids) = fd.get_array(&CosName::new(b"Kids")) {
            for k in kids.to_vec() {
                queue.push(k);
            }
        }
        let is_sig = fd
            .get_name(&CosName::new(b"FT"))
            .map(|n| n.as_bytes() == b"Sig")
            .unwrap_or(false);
        if !is_sig {
            continue;
        }
        ...
        let (sig_id, is_ts) = match fd.get(&CosName::new(b"V")).and_then(|v| v.as_reference()) {
            Some(vr) => {
                let is_ts = doc
                    ...
                    .map(|vd| {
                        vd.get_name(&CosName::new(b"Type"))
                            .map(|n| n.as_bytes() == b"DocTimeStamp")
                            .unwrap_or(false)
                            || vd
                                .get_name(&CosName::new(b"SubFilter"))
                                .map(|n| n.as_bytes() == b"ETSI.RFC3161")
                                .unwrap_or(false)
                    })
                    .unwrap_or(false);
                (vr, is_ts)
...
fn validate_one(
    ...
    doc: &Document,
    field: SigField,
) -> Result<ValidationResult, PdfError> {
    let mut errors: Vec<String> = Vec::new();

    let sd = match doc
        .objects
        .get(&field.sig_dict_id)
        .and_then(|o| o.as_dictionary())
    {
        Some(d) => d.clone(),
        None => {
    ...
    let byte_range: Vec<i64> = sd
        .get_array(&CosName::new(b"ByteRange"))
    ...
    let covers_file =
        byte_range.len() == 4 && (byte_range[2] + byte_range[3]) as usize == pdf_bytes.len();
    ...
    if cms_bytes.is_empty() {
        errors.push("Contents entry missing".into());
    } else if all_zeros {
        errors.push("Contents is all zeros".into());
    }
    ...
    let computed_digest: Vec<u8> = if byte_range.len() == 4 && !cms_bytes.is_empty() && !all_zeros {
        ...
        if e0 <= pdf_bytes.len() {
            h.update(&pdf_bytes[s0..e0]);
        }
        if e1 <= pdf_bytes.len() {
            h.update(&pdf_bytes[s1..e1]);
        }
        h.finalize().to_vec()
    } else {
        vec![]
    ...
    if !cms_bytes.is_empty() && !all_zeros {
        ...
        let parse_res = if is_ts {
            SignedData::parse_ber(trimmed).or_else(|_| match extract_inner_signed_data(trimmed) {
                        ...
                        if !computed_digest.is_empty() {
                            if is_ts {
                                match extract_ts_imprint(trimmed) {
                                    ...
                                    Some(_) => {
                                        errors.push("Timestamp messageImprint mismatch".into())
                                    }
                                    None => {
                                        if cms_sig_valid {
                                            digest_match = true;
                                        } else {
                                            errors.push("Could not extract messageImprint".into());
                                        }
                                    }
                                }
                            } else {
                                match extract_message_digest(&cms_bytes) {
                                    ...
                                    None => {
                                        if cms_sig_valid {
                                            ...
                                            errors.push("Could not extract messageDigest".into());
                                        }
                                    }
                                }
                            }
    ...
    let has_revoc_in_cms = oid_present(
        &cms_bytes,
        &[
    ...
    Ok(ValidationResult {
        ...
        sub_filter,
        byte_range,
        byte_range_covers_whole_file: covers_file,
        is_encrypted: false,
        computed_digest,
        digest_match,
        cms_signature_valid: cms_sig_valid,
        certificates: certs,
        certificate_chain_valid: chain_valid,
        certificate_chain_trusted: chain_trusted,
        chain_warnings,
        has_dss,
        dss_crl_count: dss_crls,
        dss_ocsp_count: dss_ocsps,
        dss_cert_count: dss_certs,
        has_vri,
        has_cms_revocation_data: has_revoc_in_cms,
        has_timestamp,
        is_ltv_enabled: is_ltv,
...
fn extract_inner_signed_data(data: &[u8]) -> Option<Vec<u8>> {
    ...
    if after >= data.len() || data[after] != 0xA0 {
        return None;
    }
...
fn extract_message_digest(cms: &[u8]) -> Option<Vec<u8>> {
    ...
    for i in 0..cms.len().saturating_sub(oid.len()) {
        if &cms[i..i + oid.len()] != oid {
            continue;
        }
        let after = i + oid.len();
        if after >= cms.len() || cms[after] != 0x31 {
            continue;
        }
        let (ss, _) = read_der_len(cms, after + 1)?;
        if ss >= cms.len() || cms[ss] != 0x04 {
            continue;
        }
...
fn extract_ts_imprint(data: &[u8]) -> Option<Vec<u8>> {
    ...
    for i in 0..data.len().saturating_sub(sha256.len()) {
        if &data[i..i + sha256.len()] != sha256 {
            continue;
        }
        let mut pos = i + sha256.len();
        // skip NULL AlgorithmIdentifier parameter if present
        if pos + 1 < data.len() && data[pos] == 0x05 && data[pos + 1] == 0x00 {
            pos += 2;
        }
        for off in 0..10 {
            let c = pos + off;
...
fn order_chain(certs: &[CertInfo]) -> Vec<CertInfo> {
    ...
    let issuer_set: HashSet<&str> = certs
        .iter()
    ...
    let ee = match ee {
        Some(e) => e,
        None => return certs.to_vec(),
    };
    let mut chain = vec![ee.clone()];
    let mut cur = &ee.issuer;
    let mut used: HashSet<usize> =
        [certs.iter().position(|c| std::ptr::eq(c, ee)).unwrap_or(0)].into();
    for _ in 0..certs.len() {
        match certs
            .iter()
            .enumerate()
            .find(|(i, c)| !used.contains(i) && c.subject == *cur)
        {
            Some((idx, next)) => {
...
fn check_chain(certs: &[CertInfo], _now: &DateTime<Utc>) -> (bool, bool, Vec<String>) {
    ...
    if chain.len() > 1 {
        for i in 0..chain.len() - 1 {
            if chain[i].issuer != chain[i + 1].subject {
                valid = false;
                w.push(format!(
                    "Chain break: cert[{i}] issuer '{}' ≠ cert[{}] subject '{}'",
                    chain[i].issuer,
                    i + 1,
                    chain[i + 1].subject
                ));
    ...
    for (i, c) in chain.iter().enumerate() {
        if c.is_expired {
            valid = false;
            w.push(format!("Cert [{i}] '{}' expired", c.subject));
        }
    }
    ...
    let trusted = if root.is_self_signed {
        if chain.len() == 1 {
            ...
            if !t {
                w.push(format!(
                    "Root '{}' self-signed, not recognized as public CA",
                    root.subject
                ));
            }
            t
        ...
        if !t {
            w.push(format!(
                "Root issuer '{}' not recognized as trusted CA",
                root.issuer
            ));
        }
        t
...
fn detect_modifications(pdf: &[u8], results: &mut Vec<ValidationResult>) -> Result<(), PdfError> {
    ...
    for i in 0..n {
        if i == last && results[last].byte_range_covers_whole_file {
            continue;
        }
        ...
        let rev_doc = match Document::load_from_bytes(&pdf[..rev_end]) {
            ...
            Err(_) => {
                ...
                results[i]
                    .modification_notes
                    .push("Could not parse revision for modification check".into());
                continue;
            }
        };
        ...
        if unauth {
            results[i]
                .errors
                .push("Document modified after this signature was applied".into());
        }
    }
    Ok(())
}
...
fn compare_revisions(rev: &Document, full: &Document) -> (bool, Vec<String>) {
    ...
    for &id in full_ids.difference(&rev_ids) {
        match classify_new(full, id) {
            Change::Permitted(d) => {
                notes.push(format!("Added obj {}: {} (permitted)", id.object_number, d))
            }
            Change::Unauthorized(d) => {
                notes.push(format!(
                    "Added obj {}: {} (UNAUTHORIZED)",
                    id.object_number, d
                ));
                unauth = true;
            }
    ...
    for &id in rev_ids.intersection(&full_ids) {
        let ro = match rev.objects.get(&id) {
            Some(o) => o,
            None => continue,
        };
        let fo = match full.objects.get(&id) {
            Some(o) => o,
            None => continue,
        };
        if format!("{ro:?}") == format!("{fo:?}") {
            continue;
        }
        match classify_modified(full, id, ro, fo) {
            Change::Permitted(d) => notes.push(format!(
                "Modified obj {}: {} (permitted)",
                id.object_number, d
            )),
            Change::Unauthorized(d) => {
                notes.push(format!(
                    "Modified obj {}: {} (UNAUTHORIZED)",
                    id.object_number, d
                ));
                unauth = true;
            }
...
fn classify_new(doc: &Document, id: ObjectId) -> Change {
    let obj = match doc.objects.get(&id) {
        Some(o) => o,
        None => return Change::Unauthorized("unreadable".into()),
    };
    match obj {
        CosObject::Dictionary(d) => {
                    ...
                    b"Annot" => {
                        if let Some(sub) = d.get_name(&CosName::new(b"Subtype")) {
                            ...
                            if matches!(
                                ...
                                return Change::Unauthorized(format!(
                                    "[EAA] dangerous annotation /{}",
            ...
            if d.get_name(&CosName::new(b"FT"))
                .map(|n| n.as_bytes() == b"Sig")
                .unwrap_or(false)
            {
                return Change::Permitted("signature field".into());
            }
            if d.get(&CosName::new(b"VRI")).is_some()
                || d.get(&CosName::new(b"CRLs")).is_some()
                || d.get(&CosName::new(b"OCSPs")).is_some()
                || d.get(&CosName::new(b"Certs")).is_some()
            {
                return Change::Permitted("DSS dictionary".into());
            }
            if d.get(&CosName::new(b"Cert")).is_some()
                || d.get(&CosName::new(b"CRL")).is_some()
                || d.get(&CosName::new(b"OCSP")).is_some()
            {
                return Change::Permitted("VRI entry".into());
            }
            if d.get(&CosName::new(b"ByteRange")).is_some()
                && d.get(&CosName::new(b"Contents")).is_some()
            {
                return Change::Permitted("signature value dict".into());
            }
        ...
        CosObject::Stream(s) => {
            ...
            if let Some(tn) = d.get_name(&CosName::new(b"Type")) {
                match tn.as_bytes() {
                    ...
                    b"ObjStm" => {
                        return Change::Unauthorized(
                            "[Shadow] object stream added after signing".into(),
                        );
                    }
                    b"XObject" => {
                        if let Some(sub) = d.get_name(&CosName::new(b"Subtype")) {
                            if sub.as_bytes() == b"Form" {
                                return Change::Permitted("form XObject (sig appearance)".into());
                            }
                            if sub.as_bytes() == b"Image" {
                                return Change::Unauthorized(
            ...
            if d.get(&CosName::new(b"Resources")).is_some()
                || d.get(&CosName::new(b"BBox")).is_some()
            {
                return Change::Unauthorized(
            ...
            if keys
                .iter()
                .all(|k| matches!(k.as_str(), "Length" | "Filter" | "DecodeParms" | "DL"))
            {
...
fn classify_modified(full: &Document, id: ObjectId, ro: &CosObject, fo: &CosObject) -> Change {
    if let Some(cat_id) = full.catalog_ref() {
        if cat_id == id {
            return classify_catalog_change(ro, fo);
        }
    }
    if let (Some(rd), Some(fd)) = (ro.as_dictionary(), fo.as_dictionary()) {
        if fd
            .get_name(&CosName::new(b"Type"))
        ...
        if fd
            .get_name(&CosName::new(b"Type"))
            .map(|n| n.as_bytes() == b"Pages")
            .unwrap_or(false)
        {
            return Change::Unauthorized("[Shadow] /Type /Pages tree modified".into());
        }
        if fd.get(&CosName::new(b"Fields")).is_some()
            && fd.get(&CosName::new(b"SigFlags")).is_some()
        {
...
fn classify_catalog_change(ro: &CosObject, fo: &CosObject) -> Change {
    ...
    for (k, fv) in fd.iter() {
        ...
        match rd.get(k) {
            ...
            Some(_) => match ks.as_str() {
                ...
                "OCProperties" | "Pages" => {
                    changes.push(format!("[Shadow] /{ks} modified"));
                    unauth = true;
                }
                _ => {
                    changes.push(format!("/{ks} modified (unauthorized)"));
                    unauth = true;
                }
            },
            None => match ks.as_str() {
                ...
                "OCProperties" => {
                    changes.push("[Shadow] /OCProperties added".into());
                    unauth = true;
                }
                _ => {
                    changes.push(format!("/{ks} added (unauthorized)"));
                    unauth = true;
                }
            },
        }
    ...
    let desc = if changes.is_empty() {
        "catalog (no changes)".into()
    } else {
        format!("catalog: {}", changes.join(", "))
    };
    if unauth {
        Change::Unauthorized(desc)
    } else {
        Change::Permitted(desc)
    }
}
    ...
    for (k, fv) in fd.iter() {
        ...
        match rd.get(k) {
            ...
            Some(rv) => match ks.as_str() {
                "Annots" => {
                    if array_append_only(rv, fv) {
                        changes.push("/Annots extended".into());
                    } else {
                        changes.push("[EAA] /Annots modified (not append-only)".into());
                        unauth = true;
                    }
                }
                "Resources" => changes.push("/Resources updated".into()),
                "Contents" => {
                    changes.push("[Shadow] /Contents reference changed".into());
                    unauth = true;
                }
            ...
            Some(rv) => match ks.as_str() {
                "Fields" => {
                    if array_append_only(rv, fv) {
                        ...
                        changes.push("/Fields modified (not append-only)".into());
                        unauth = true;
                    }
                }
...
fn array_append_only(rv: &CosObject, fv: &CosObject) -> bool {
    let ra = match rv.as_array() {
        Some(a) => a,
        None => return false,
    };
    let fa = match fv.as_array() {
        Some(a) => a,
        None => return false,
    };
    fa.len() >= ra.len()
...
fn usf_check(pdf: &[u8], br: &[i64]) -> (bool, Vec<String>) {
    ...
    if o1 != 0 {
        w.push(format!("[USF] ByteRange starts at {o1} not 0"));
        ok = false;
    }
    if o1 < 0 || l1 < 0 || o2 < 0 || l2 < 0 {
        w.push("[USF] Negative ByteRange values".into());
        ok = false;
    }
    if o1 + l1 > flen {
        w.push("[USF] First range exceeds file".into());
        ok = false;
    }
    if o2 + l2 > flen {
        w.push("[USF] Second range exceeds file".into());
        ok = false;
    }
    let end0 = o1 + l1;
    if end0 > o2 {
        w.push(format!("[USF] Ranges overlap (end0={end0} > o2={o2})"));
        ok = false;
    }
    if end0 < o2 && end0 >= 0 && o2 <= flen {
        ...
        let gap = &pdf[gs..ge];
        if gap.is_empty() {
            w.push("[USF] Zero-length gap".into());
            ok = false;
        } else {
            if gap[0] != b'<' {
                w.push("[USF] Gap doesn't start with '<'".into());
                ok = false;
            }
            if gap[gap.len() - 1] != b'>' {
                w.push("[USF] Gap doesn't end with '>'".into());
                ok = false;
            }
            if gap.len() >= 2 && gap[1..gap.len() - 1].iter().any(|b| !b.is_ascii_hexdigit()) {
                w.push("[USF] Gap contains non-hex chars".into());
                ok = false;
            }
        }
    }
    if l1 < 50 {
        w.push(format!(
            "[USF] First segment suspiciously short ({l1} bytes)"
        ));
        ok = false;
    }
    (ok, w)
...
fn swa_check(pdf: &[u8], br: &[i64]) -> (bool, Vec<String>) {
    ...
    if gs >= pdf.len() || ge > pdf.len() || gs >= ge {
        return (true, w);
    }
    ...
    if count > 10 {
        w.push(format!(
            "[SWA] Found {count} large /Contents hex-strings (possible wrapping)"
        ));
        return (false, w);
    }
    (true, w)
...
fn certification_check(
    ...
    let cat = match doc.catalog() {
        Some(c) => c.clone(),
        None => return (None, true, w),
    };
    let perms_obj = match cat.get(&CosName::new(b"Perms")) {
        Some(v) => v.clone(),
        None => return (None, true, w),
    };
    ...
    let docmdp_ref = match perms
        .get(&CosName::new(b"DocMDP"))
    ...
    if let Some(level) = extract_mdp(doc, &sig_dict) {
        w.push(format!(
            "[Certification] MDP level {level} — {}",
            match level {
                1 => "no changes",
                2 => "form fill-in + signing",
                3 => "form + signing + annots",
                _ => "unknown",
            }
        ));
        let ok = mdp_compliant(level, result);
        if !ok {
            w.push(format!("[Certification] Changes VIOLATE MDP level {level}"));
        }
...
fn extract_mdp(doc: &Document, sig: &CosDictionary) -> Option<u8> {
    ...
    for item in refs {
        ...
        if !rd
            .get_name(&CosName::new(b"TransformMethod"))
            .map(|n| n.as_bytes() == b"DocMDP")
            .unwrap_or(false)
        {
            continue;
        }
        let tp_obj = match rd.get(&CosName::new(b"TransformParams")) {
            Some(v) => v.clone(),
            None => continue,
        };
        let tp = match tp_obj {
        ...
        if let Some(p) = tp.get_int(&CosName::new(b"P")) {
            return Some(p as u8);
        }
    }
    None
...
fn mdp_compliant(level: u8, r: &ValidationResult) -> bool {
    ...
    match level {
        ...
        2 => r
            .modification_notes
            .iter()
            .all(|n| allow(n) || n.to_lowercase().contains("annots extended")),
        3 => r.modification_notes.iter().all(|n| {
            allow(n) || n.to_lowercase().contains("annot") || n.to_lowercase().contains("widget")
        }),
        _ => true,
    }
}
