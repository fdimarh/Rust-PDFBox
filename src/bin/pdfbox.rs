...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        Commands::ExtractText { input, output } => {
            if cli.verbose {
                eprintln!("ℹ Loading document: {}", input.display());
            }
        ...
        Commands::Sign {
            ...
            if cli.verbose {
                eprintln!("ℹ Loading PDF: {}", input.display());
            }
            ...
            let sig_image: Option<Vec<u8>> = if let Some(img_path) = image {
                if cli.verbose {
                    eprintln!("ℹ Loading signature image: {}", img_path.display());
                }
                // Read as raw bytes — the signing pipeline may embed them.
                // For now, warn that --image is not yet integrated into sign_pdf.
                eprintln!(
                    "⚠ --image flag recognized but image embedding in signature appearance is not yet implemented; signing will proceed without visual."
                );
                None
            } else {
                None
            ...
            if cli.verbose {
                eprintln!(
                    "ℹ Signing PDF (format={:?}, page={page}, rect={rect_parsed:?}) ...",
                    opts.format
                );
            }
            let signed =
                signing::sign_pdf(&pdf_bytes, &cert_pem, &key_pem, password.as_deref(), &opts)
        ...
        Commands::Validate { input, check_ltv } => {
            ...
            #[cfg(feature = "signing")]
            {
                ...
                for (i, sig) in result.signatures.iter().enumerate() {
                    ...
                    println!(
                        "      Signer: {}",
                        sig.signer_name.as_deref().unwrap_or("(unknown)")
                    );
                    println!(
                        "      Reason: {}",
                        sig.reason.as_deref().unwrap_or("(not specified)")
                    );
                    if *check_ltv {
                        let ltv = if sig.is_ltv_enabled {
                            "✅ LTV enabled"
                        } else {
                            "❌ Not LTV"
                        };
                        ...
                        println!(
                            "      DSS CRLs: {}, OCSPs: {}",
                            sig.dss_crl_count, sig.dss_ocsp_count
                        );
                    }
                }
                println!(
        ...
        Commands::Preflight { input, output } => {
            ...
            if let Some(out_path) = output {
                ...
                std::fs::write(out_path, &json)
                    .map_err(|e| format!("Failed to write '{}': {e}", out_path.display()))?;
                if cli.verbose {
                    eprintln!("ℹ JSON output written to {}", out_path.display());
                }
            }
        }
        ...
        Commands::Protect {
            ...
            let policy = rust_pdfbox::protection::StandardProtectionPolicy {
                ...
                owner_password: owner_password
                    .clone()
                    .unwrap_or_else(|| format!("rust-pdfbox-{}", std::process::id())),
