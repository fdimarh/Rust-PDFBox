//! RC4 stream cipher — variable-length key (1–256 bytes).
//!
//! PDF Standard Security Handler uses RC4 with 5-byte (Rev 2) and 16-byte
//! (Rev 3) keys. The RustCrypto `rc4` crate requires a fixed generic key size,
//! which cannot handle PDF's variable-length keys, so we keep a minimal
//! custom implementation here.
//!
//! Maps to Java PDFBox `ARCFourEncryption`.
