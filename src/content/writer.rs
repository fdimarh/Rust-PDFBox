...
impl<'a> ContentStreamWriter<'a> {
    ...
    pub fn show_text_positioned<'b>(&mut self, elements: &[TextShowElement<'b>]) -> PdfResult<()> {
        ...
        for element in elements {
            ...
            first = false;
            match element {
                TextShowElement::Text(text) => {
                    Self::push_pdf_literal_string(&mut self.buffer, text)
                }
                TextShowElement::Adjust(amount) => {
                    self.buffer
                        .extend_from_slice(format!("{}", amount).as_bytes());
                }
            }
        }
        self.buffer.extend_from_slice(b"] TJ\n");
    ...
    pub fn register_form_xobject(
        ...
        self.doc
            .insert_object(form_id, CosObject::Stream(form_stream));
    ...
    pub fn register_image_xobject_flate_rgb8(
        ...
        self.register_image_xobject_flate_rgb8_with_decode_parms(
            name_hint, width, height, flate_data, None,
        )
    }
    ...
    fn register_image_xobject_core(
        ...
        let smask_id = if let Some(alpha) = smask_data {
            ...
            self.doc
                .insert_object(smask_id, CosObject::Stream(smask_stream));
            self.doc.xref.insert_if_absent(
                smask_id,
                crate::parser::xref::XRefEntry::InUse {
                    offset: 0,
        ...
        image_dict.insert(CosName::new(b"ColorSpace".to_vec()), color_space);
        image_dict.insert(
            CosName::new(b"BitsPerComponent".to_vec()),
            CosObject::Integer(8),
        );
        image_dict.insert(
            CosName::length(),
            CosObject::Integer(image_data.len() as i64),
        );
        ...
        self.doc
            .insert_object(image_id, CosObject::Stream(image_stream));
        self.doc.xref.insert_if_absent(
            image_id,
            crate::parser::xref::XRefEntry::InUse {
                offset: 0,
    ...
    fn insert_xobject_resource(
        resources_dict: &mut CosDictionary,
        image_name: &str,
        image_id: ObjectId,
    ) {
            ...
            if let Some(resources_obj) = current_dict.get(&CosName::resources()) {
                let found = match resources_obj {
                    ...
                    CosObject::Reference(resources_id) => self
                        ...
                        .get_object_ref(*resources_id)
                        .and_then(|obj| obj.as_dictionary())
                        .and_then(|resources_dict| {
                            resources_dict.get(&CosName::new(b"XObject".to_vec()))
                        })
                        .and_then(|obj| obj.as_dictionary())
                        .map(|xobjects| xobjects.contains_key(&target))
                        .unwrap_or(false),
    ...
    pub fn close(self) -> PdfResult<()> {
        ...
        dict.insert(
            CosName::new(b"Length".to_vec()),
            CosObject::Integer(self.buffer.len() as i64),
        );
