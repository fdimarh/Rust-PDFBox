...
fn main() {
            ...
            "--mode" => {
                ...
                if i < args.len() {
                    mode = match args[i].as_str() {
                        ...
                        other => {
                            eprintln!(
                                "Unknown mode '{}'; valid: extreme, recommended, less",
                                other
                            );
                            process::exit(1);
                        }
    ...
    if no_images {
        opts.optimize_images = false;
        opts.optimize_png_images = false;
    }
    if no_fonts {
        opts.subset_fonts = false;
        opts.font_remove_unused = false;
    }
    if no_cmyk {
    ...
    if let Some(q) = jpeg_quality {
        opts.image_jpeg_quality = q;
    }
    if let Some(d) = max_dpi {
        opts.image_max_dpi = d;
    }
    ...
    let mut doc = match Document::load_from_bytes(&raw) {
        Ok(d) => {
            println!("ok ({} bytes, strict)", d.source_len());
            d
        }
        Err(strict_err) => {
    ...
    println!();
    println!(
        "  Streams recompressed : {:>6}",
        report.streams_recompressed
    );
    println!("  Images resampled     : {:>6}", report.images_resampled);
    println!(
        "  Images PNG optimized : {:>6}",
        report.images_png_optimized
    );
    println!(
        "  Images →CCITT G4     : {:>6}",
        report.images_ccitt_encoded
    );
    println!(
        "  Images CMYK→sRGB     : {:>6}",
        report.images_cmyk_converted
    );
    println!(
        "  Images →gray         : {:>6}",
        report.images_grayscale_converted
    );
    ...
    println!(
        "  Errors skipped       : {:>6}",
        report.decode_errors_skipped
    );
    println!();
    println!(
        "  Saved (streams)      : {:>10} bytes",
        report.bytes_saved_streams
    );
    println!(
        "  Saved (images)       : {:>10} bytes",
        report.bytes_saved_images
    );
    println!(
        "  Saved (fonts)        : {:>10} bytes",
        report.bytes_saved_fonts
    );
    println!(
        "  Saved (dedup)        : {:>10} bytes",
        report.bytes_saved_dedup
    );
    println!(
        "  Saved (metadata)     : {:>10} bytes",
        report.bytes_saved_metadata
    );
    println!("────────────────────────────────────────────────────────────────");
}
