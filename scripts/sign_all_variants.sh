...
# Like run() but uses a custom input PDF instead of $IN.
# Usage: run_with <label> <input.pdf> [extra args…]
run_with() {
  local label="$1"; shift
  local custom_in="$1"; shift
  local out="$OUT/${label}.pdf"
  printf '\n════════════════════════════════════════════════════════\n'
  printf '▶  [%s]\n' "$label"
  printf '   Input: %s\n' "$custom_in"
  printf '   Args: %s\n\n' "$*"
  if "$BIN" "$custom_in" -c "$CERT" -k "$KEY" -o "$out" "$@"; then
    PASS=$((PASS+1))
    printf '\n   ✅  Output: %s  (%d bytes)\n' "$out" "$(wc -c < "$out")"
  else
    FAIL=$((FAIL+1))
...
# ════════════════════════════════════════════════════════
# ANCHOR-TAG SIGNING — uses anchor_sample.pdf
# Generated by build_anchor_sample_pdf() in the Rust test
# suite; run `cargo test anchor_image_sign_demo_writes_valid_pdf`
# or `cargo test diag_byterange_alignment` first if the file
# does not yet exist, or let this script generate it automatically.
# ════════════════════════════════════════════════════════
ANCHOR_IN="tests/signing_assets/anchor_sample.pdf"
ANCHOR_IMG="tests/signing_assets/sig1.png"

# Generate anchor_sample.pdf if it is missing
if [ ! -f "$ANCHOR_IN" ]; then
  printf '\n════════════════════════════════════════════════════════\n'
  printf '  Generating %s via cargo test …\n' "$ANCHOR_IN"
  cargo test --features text anchor_image_sign_demo_writes_valid_pdf -- --nocapture 2>/dev/null \
    || true
fi

if [ ! -f "$ANCHOR_IN" ]; then
  printf '  ⚠  %s not found — skipping anchor-tag variants.\n' "$ANCHOR_IN"
else

  # ── Anchor invisible ──────────────────────────────────────────────────
  run_with "65_anchor_invisible" "$ANCHOR_IN" \
    -f pkcs7 --invisible --no-tsa \
    --name "Rust PDFBox Demo Signer" \
    --contact "demo@rust-pdfbox.local" \
    --location "Jakarta, Indonesia" \
    --reason "Anchor invisible demo" \
    --field "AnchorSignature" \
    --reserved 32768

  # ── Anchor tag InFront — text-only appearance ─────────────────────────
  run_with "66_anchor_tag_infront_text" "$ANCHOR_IN" \
    -f pkcs7 --no-tsa \
    --tag "##SIGN_HERE##" --width 200 --height 60 --tag-mode front \
    --name "Rust PDFBox Demo Signer" \
    --contact "demo@rust-pdfbox.local" \
    --location "Jakarta, Indonesia" \
    --reason "Anchor InFront text-only" \
    --field "AnchorSignature" \
    --reserved 32768

  # ── Anchor tag Overlay — text-only appearance ─────────────────────────
  run_with "67_anchor_tag_overlay_text" "$ANCHOR_IN" \
    -f pkcs7 --no-tsa \
    --tag "##SIGN_HERE##" --width 200 --height 60 --tag-mode overlay \
    --name "Rust PDFBox Demo Signer" \
    --contact "demo@rust-pdfbox.local" \
    --location "Jakarta, Indonesia" \
    --reason "Anchor Overlay text-only" \
    --field "AnchorSignature" \
    --reserved 32768

  # ── Anchor tag InFront — image appearance (if sig1.png present) ───────
  if [ -f "$ANCHOR_IMG" ]; then
    run_with "68_anchor_tag_infront_image" "$ANCHOR_IN" \
      -f pkcs7 --no-tsa \
      --tag "##SIGN_HERE##" --width 200 --height 60 --tag-mode front \
      --image "$ANCHOR_IMG" \
      --name "Rust PDFBox Demo Signer" \
      --contact "demo@rust-pdfbox.local" \
      --location "Jakarta, Indonesia" \
      --reason "Approved — anchor-tag signing demo" \
      --field "AnchorSignature" \
      --reserved 32768
  else
    printf '  ⚠  %s not found — skipping image anchor variant 68.\n' "$ANCHOR_IMG"
  fi

  # ── Anchor tag InFront — PAdES B-B ────────────────────────────────────
  run_with "69_anchor_tag_pades_bb" "$ANCHOR_IN" \
    -f pades -l b-b --no-tsa \
    --tag "##SIGN_HERE##" --width 200 --height 60 --tag-mode front \
    --name "Rust PDFBox Demo Signer" \
    --contact "demo@rust-pdfbox.local" \
    --location "Jakarta, Indonesia" \
    --reason "Anchor PAdES B-B" \
    --field "AnchorSignature" \
    --reserved 32768

  # ── Anchor tag InFront — PAdES B-T (live TSA) ─────────────────────────
  run_with "70_anchor_tag_pades_bt" "$ANCHOR_IN" \
    -f pades -l b-t \
    --tsa "$TSA" \
    --tag "##SIGN_HERE##" --width 200 --height 60 --tag-mode front \
    --name "Rust PDFBox Demo Signer" \
    --contact "demo@rust-pdfbox.local" \
    --location "Jakarta, Indonesia" \
    --reason "Anchor PAdES B-T" \
    --field "AnchorSignature" \
    --reserved 32768

  # ── Anchor + image — visible on real sample.pdf (fixed rect) ──────────
  if [ -f "$ANCHOR_IMG" ]; then
    run_with "71_anchor_real_sample_image_rect" "$IN" \
      -f pkcs7 --no-tsa \
      --rect "50,680,280,740" \
      --image "$ANCHOR_IMG" \
      --name "Rust PDFBox Demo Signer" \
      --contact "demo@rust-pdfbox.local" \
      --location "Jakarta, Indonesia" \
      --reason "Visible image on real sample.pdf" \
      --reserved 32768
  fi
